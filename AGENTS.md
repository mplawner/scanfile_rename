...
2) If needed, render pages via Poppler `pdftoppm` and call an OpenAI-compatible LLM endpoint (LM Studio / Ollama examples)
3) Build a sanitized filename and copy/move the PDF into an output directory

The tool may also (best-effort) enrich classic PDF DocumentInfo metadata on the output file; it skips metadata writing when the PDF appears encrypted or signed.
...
An OpenAI-compatible LLM server must be running and reachable via `LLM_ENDPOINT` (legacy alias: `LM_STUDIO_ENDPOINT`).
...
python3 scanfile_rename.py "scan.pdf" --dry-run
python3 scanfile_rename.py "scan.pdf" --print-json
python3 scanfile_rename.py "scan.pdf" --keywords-count 10
python3 scanfile_rename.py "scan.pdf" --no-progress
python3 scanfile_rename.py "scan.pdf" --no-repair
python3 scanfile_rename.py "scan.pdf" --lm-timeout 120 --lm-retries 1
python3 scanfile_rename.py --version
```

## Environment Variables

- `LLM_ENDPOINT` (default: `http://localhost:1234/v1/chat/completions`)
- `LLM_MODEL` (default: `qwen3-vl-8b-instruct`)
- `LLM_TIMEOUT` (default: `120` seconds)
- `LLM_MAX_RETRIES` (default: `0`)
- Legacy aliases supported: `LM_STUDIO_ENDPOINT`, `LM_STUDIO_MODEL`, `LM_STUDIO_TIMEOUT`, `LM_STUDIO_MAX_RETRIES`
